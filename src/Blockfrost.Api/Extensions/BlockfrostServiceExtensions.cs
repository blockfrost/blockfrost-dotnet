            ...
            return services.AddHttpClient<TClient, TImplementation>($"{name}:{typeof(TImplementation).Name}", (client, provider) =>
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var health = CreateClient<Api.Services.IHealthService, Api.Services.HealthService>(network, client);
                        var metrics = CreateClient<Api.Services.IMetricsService, Api.Services.MetricsService>(network, client);
